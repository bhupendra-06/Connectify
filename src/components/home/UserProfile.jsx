        ...
        <div className="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6 border-b pb-6 sm:pt-4">
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex flex-col md:flex-row items-center md:items-start space-y-2 md:space-y-0 md:space-x-4 mb-4">
              ...
              {myProfile.data._id === MyOwnerId ? (
                <button className="px-4 py-2 text-sm font-semibold bg-white border border-gray-400 rounded-lg" onClick={openEditModal}>
                  Edit Profile
                </button>
              ) : (
                <button
                  ...
                  disabled={loading}
                  className={`px-4 py-2 text-sm font-semibold rounded-lg ${isFollowing
        ...
        {showEditModal && (
          <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center">
            <div className="bg-white p-6 rounded-lg w-96">
              ...
              <input type="text" value={updatedProfile.username} onChange={(e) => setUpdatedProfile({ ...updatedProfile, username: e.target.value })} className="w-full border p-2 mb-2" />
              <label className="font-semibold">Full Name</label>
              <input type="text" value={updatedProfile.fullName} onChange={(e) => setUpdatedProfile({ ...updatedProfile, fullName: e.target.value })} className="w-full border p-2 mb-2" />
              <label className="font-semibold">Bio</label>
              <textarea value={updatedProfile.bio} onChange={(e) => setUpdatedProfile({ ...updatedProfile, bio: e.target.value })} className="w-full border p-2 mb-2 resize-none h-20 "></textarea>
              <label className="font-semibold">Profile Image</label>
              <input
                type="file"
                onChange={(e) => setUpdatedProfile({ ...updatedProfile, avatar: e.target.files[0] })}
                className="w-full border p-2 mb-2"
              />
              <label className="font-semibold">Cover Image</label>
              <input
                type="file"
                onChange={(e) => setUpdatedProfile({ ...updatedProfile, coverImage: e.target.files[0] })}
                className="w-full border p-2 mb-2"
              />
              <label className="font-semibold">New Password</label>
              <input type="password" value={updatedProfile.password} onChange={(e) => setUpdatedProfile({ ...updatedProfile, password: e.target.value })} className="w-full border p-2 mb-4" placeholder="Enter new password" />
              <button onClick={handleProfileUpdate} className="px-4 py-2 bg-primaryColor text-white rounded" disabled={loading}>  {loading ? (
                <ClipLoader size={20} color="#ffffff" className="mx-5" />
              ) : (
                "Save Changes"
              )}</button>
              <button onClick={() => setShowEditModal(false)} className="ml-2 px-4 py-2 bg-gray-500 text-white rounded">Cancel</button>
            </div>
          </div>
        )}
      </div>
